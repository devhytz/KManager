...
def search_book_isbn():
    ...
    found = books.searching(isbn)  # Primero verifica que exista
    if not found:
        messagebox.showwarning("Not found", f"No book with ISBN {isbn}")
        return
    
    book = books.binary_search(isbn)  # Luego obtén el libro ordenado
...
def show_inefficient_shelving():
    try:
        ...
        # Ejecutar el algoritmo y capturar la salida
        import io
        import sys

        buffer = io.StringIO()
        sys.stdout = buffer  # Redirige print a un buffer

        books.inefficient_shelving()  # Ejecuta tu función original
...
def show_efficient_shelving():
    try:
        ...
        buffer = io.StringIO()
        sys.stdout = buffer  # Captura los prints

        books.efficient_shelving()  # Ejecuta el algoritmo original
